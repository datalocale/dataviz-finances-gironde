...
export default class SunburstSlice extends React.Component {
  shouldComponentUpdate(nextProps) {
    if (
      ['radius', 'donutWidth', 'startAngle', 'endAngle', 'selectedNode'].some(
        k => this.props[k] !== nextProps[k]
      )
    )
      return true;

    if (this.props.node !== nextProps.node) return true;
    ...
    if (this.props.highlightedNodes === nextProps.highlightedNodes)
      ...
      return nodes.some(
        n =>
          this.props.highlightedNodes.has(n) !==
            nextProps.highlightedNodes.has(n)
      );
    }
  }
  ...
  render() {
    ...
    const pie = d3Shape.pie().startAngle(startAngle).endAngle(endAngle);
    const arc = d3Shape
      .arc()
