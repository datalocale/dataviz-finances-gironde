...
function makeUnusedM52RowsSet(aggregatedInstruction, M52Instruction) {
  return M52Instruction.filter(m52row => {
    return !aggregatedInstruction.some(aggRow =>
      aggRow['M52Rows'].has(m52row)) &&
      isOR(m52row);
  });
}
...
export default class TextualSelected extends React.PureComponent {
  render() {
    ...
    const unusedM52Set = makeUnusedM52RowsSet(
      aggregatedInstruction,
      M52Instruction
    );
    const usedMoreThanOnceM52RowsSet = makeUsedMoreThanOnceM52RowsSet(
      aggregatedInstruction,
      M52Instruction
    );
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          aggregatedInstruction.map(aggRow => React.createElement(
            'tr',
            {
              key: aggRow['id'],
              className: [
            ...
            React.createElement(
              ...
              format(aggRow['Montant'], { code: 'EUR' })
            ),
      ...
      React.createElement(
        ...
        React.createElement(
          'h1',
          {},
          "Lignes M52 utilisées dans aucune formule d'aggrégation (" +
            unusedM52Set.size +
            ')'
        ),
        React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          'h1',
          {},
          "Lignes M52 utilisées dans au moins 2 formules d'aggrégation (" +
            usedMoreThanOnceM52RowsSet.size +
            ')'
        ),
        React.createElement(
          'ul',
          {},
          Array.from(usedMoreThanOnceM52RowsSet).map(([m52Row, aggSet]) => {
